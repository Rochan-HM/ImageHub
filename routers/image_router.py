...
async def setup():
    os.makedirs(os.path.dirname("./store/image/"), exist_ok=True)
    os.makedirs(os.path.dirname("./store/data/"), exist_ok=True)


@router.get("/images", tags=["Get Images"])
async def get_all_images(current_user: User = Depends(get_current_active_user)):
    ans = []
    for res in glob.iglob("./store/data/*.json"):
        with open(res, "r") as f:
            data = json.load(f)

        if data["author"] == current_user.username:
            ans.append(f"./store/image/{pathlib.Path(res).stem}.jpg")
...
async def get_single_images(
    ...
    if not os.path.exists(f"./store/image/{id}.jpg"):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="File not found"
        )

    with open(f"./store/data/{id}.json", "r") as f:
        data = json.load(f)
        if data["author"] != current_user.username:
            raise HTTPException(status.HTTP_401_UNAUTHORIZED)

    return FileResponse(os.path.join(os.getcwd(), "store", "image", f"{id}.jpg"))
...
async def upload_many(
    ...
    try:
        for file in files:
            ...
            with open(f"./store/image/{file_name}.jpg", "wb") as buf:
                shutil.copyfileobj(file.file, buf)

            with open(f"./store/data/{file_name}.json", "w") as f:
                json.dump(
